...
class TestNearest(tests.IrisTest):
    ...
    def setUp(self):
        ...
        self.single_point_iy = \
            np.where(cube.coord('latitude').points == y_val)[0][0]
        self.single_point_ix = \
            np.where(cube.coord('longitude').points == x_val)[0][0]
        # Use slightly-different values to test nearest-neighbour operation.
        self.single_sample_point = [('latitude', [y_val + 19.23]),
    ...
    def test_aux_coord_both_interp_dims(self):
        ...
        cube = self.test_cube
        cube.add_aux_coord(AuxCoord([[11, 12, 13, 14],
                                     [21, 22, 23, 24],
                                     [31, 32, 33, 34]], long_name='aux_xy'),
                           (1, 2))
